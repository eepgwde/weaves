...
o: $[0 = count key `.sys; .Q.opt .z.x; .sys.i.args ]
...
if[(any`semicolon`sc in key o)&any`tab`tb in key o;-2"delimiter: -tab OR -semicolon (default \",\")";.sys.exit 1]
...
if[0=hcount LOADFILE;-2"empty file: ",first .Q.x;.sys.exit 1]
sample:last head:read0(LOADFILE;0;1+last where 0xa=read1(LOADFILE;0;WIDTHHDR))
if[not DELIM in first head;-2"delimiter \"",DELIM,"\" not found in first row";.sys.exit 1]
...
if[0=nas;-2"empty file: ",first .Q.x;.sys.exit 1]
...
zh0:{$[(count distinct r)=count r:`$"}"vs 1_x[where(x:raze"}",'nameltrim string x)in"}",.Q.an];r;'`hdrs.not.distinct0]} / remove junk chars, leading underscores and spaces, preserve case 
info:update c:zh0 c from info
zh1:{$[(count distinct r)=count r:`$"}"vs 1_x[where(x:raze"}",'string lower x)in"}",.Q.an except"_"];r;'`zaphdrs.not.distinct1]} / lowercase and remove underscores
...
savescript:{refresh[];f:`$":",(string LOADNAME),".load.q";f 1:"";hs:neg hopen f;
	...
	hs"if[`help in key o;-1\"usage: q ",(string LOADNAME),".load.q [FILE(default",(string LOADFILE),")] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-af|asfile] [-exit]\\n\";exit 1]";
	...
	hs"if[any`af`asfile in key o;-1(string`second$.z.t),\" saving <\",(1_string FILE),\"> to file \"; DATA: LOAD FILE; value \":\" sv string (SAVENAME;`DATA); save `$tname: \"/\" sv string (SAVEDB;SAVENAME) ]";
