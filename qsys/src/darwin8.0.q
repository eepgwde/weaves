...
parts: { [dt0] raze ( .date0.parts @ `date$dt0 ; .time0.parts @ `time$dt0 ) }
...
isinf0: { [x] { (upper x) like "0W*" } string abs x }

// .sch.isinf @ ( 0Wt,(2#1.0f),0Wt,(2#1h) )
isinf: { [x] $[ 0 > type x; .sch.isinf0[x]; { .sch.isinf0[x] } each x] }

// Lists only
inf2null0: { [v] x: .sch.isinf[v]; if[not any x; :v];
	    t0: { [x;y] (abs type x)$0N } each v where x;
	    v[where x]:t0; v }
...
// Replace infinity with a value and the sign of the infinity
inf2v: { [v;x0] x: .sch.isinf[v]; if[not any x; :v];
